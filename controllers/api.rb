class ApiApp < App
  ...
  post '/signin' do
    sleep(3)
    halt(404, { :error => "email and/or password required" }) if params[:email].blank? || params[:password].blank?
    ...
    halt(403, { :user => user, :error => "invalid auth" }.to_json) if user.blank? || false
